...
const VoiceAgent: React.FC<VoiceAgentProps> = ({ className = "" }) => {
  ...
  const processQuery = useCallback((query: string) => {
    ...
    if (lowerQuery.includes('name') || lowerQuery.includes('who are you') || lowerQuery.includes('introduce')) {
      agentResponse = `Hello! I'm Hemanth's personal AI assistant. Hemanth Saragadam is a software engineer with over 5 years of experience building full-stack and backend platforms end to end. He currently works as a Software Consultant at Stratagem Consulting LLC, where he designs and documents agentic AI workflows, conducts technology stack analysis, and builds relational data models for client engagements.`;
    }
    else if (lowerQuery.includes('experience') || lowerQuery.includes('work') || lowerQuery.includes('job')) {
      agentResponse = `Hemanth has quite impressive experience! He's currently a Software Consultant at Stratagem Consulting LLC since June 2026, where he designs and documents agentic AI workflows, builds memory and state management for AI document generation, and produces relational data models and ER diagrams for client engagements. Previously, he was a Senior Software Engineer at Labs196 Innovations, a Full Stack Developer at Northeastern University, a Software Engineer at KYC Hub building compliance workflows, and a Full-Stack Engineer at DocsTime. He's also AWS Certified Solutions Architect and HashiCorp Terraform Associate certified.`;
    }
